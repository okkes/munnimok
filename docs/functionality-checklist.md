...
- [ ] Native deployment from the wizard: Android/iOS feature toggles; hosted track dispatches CI + downloads the signed .aab/.apk artifacts in-page for the store-mandated first upload; local track = LAN mode (family on the machine's 192.168.x, localhost twin kept for sign-in/CORS) + its own store channel app.munni.local — CI builds against the GitHub environment `local` and delivers via the Play internal track (NATIVE_LOCAL_CHANNEL gate, first upload manual once) and TestFlight
...
- [ ] Native toggles drive themselves: ticking Android/iOS under local flips LAN mode on and starts the CI build without further clicks (queued until GitHub connects when needed)
