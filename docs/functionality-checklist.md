...
- [ ] The local Android app trusts the family CA out of the box (CI bakes the machine's root into the build via NATIVE_FAMILY_CA_PEM); phone-side root.crt install is only for browser use (and for iOS — no in-app trust bundling there); the wizard prints the exact dashed ca.<ip-dashed>.sslip.io link
