...
- [ ] Wizard step 2 is a grid of feature tiles: brand mark, toggle switch, purpose, static tags (Recommended / Popular / Optional / App distribution / With the apps) and a live tag from the feature's credentials (Needs credentials + "Set up →" jump, Ready ✓, Skipped, Optional); Select recommended turns the recommended set on without turning anything off
- [ ] Wizard step 3 opens with the Integration health card: one line per integration the picked features need (GitHub included when it matters) with its state and a jump to its tile; Check all re-verifies every saved integration through the helper (stored values never reach the page; the registry token gained a helper-side check) and writes each verdict into its tile
- [ ] Integration tiles (grouped by kind, GitHub leading the Connections grid) fold to icon + title + "needed for" + chip and expand in place to the explainer and form; the GitHub tile survives re-renders
