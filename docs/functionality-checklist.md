...
- [ ] Store readiness is polled, not clicked: with the step-3 Play service account / ASC key stored, the wizard detects the one-time manual store upload by itself and flips per-env auto-publish (no Enable button); iOS's one manual step is the ASC New-App record — the pill, the build verdict and CI all name it with the exact bundle id (local channel: missing record = warning, not red; local iOS build numbers count seconds since 2026 like the Android versionCode)
