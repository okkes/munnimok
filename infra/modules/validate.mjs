...
export const VALIDATORS = {
  ...
  async synology(values, fetchImpl) {
    ...
    try {
      ...
      await s.logout();
      if (admin === false) return { ok: true, warn: true, admin, detail: 'DSM accepted the login but treats the session as a NON-administrator from this PC (its init data says is_admin=false) — the Control Panel APIs bootstrap needs answer 105 to such a session: check Control Panel → Security → Account (Adaptive MFA — on by default for administrators without 2FA — treats a sign-in it finds risky as a plain user\'s when the account has no e-mail for the code: switch it off for administrators, or enroll the account in 2FA) and that the administrators group is ticked and saved' };
  ...
  async 'logto-m2m'(values, fetchImpl) {
    ...
    if (gap) return { ok: false, detail: gap };
    // the platform config names the environments (no hardcoded "prod")
    const envs = platformEnvStacks('lcl');
    if (!envs.length) return { ok: false, detail: 'no local environment exists yet — Set up & start munni first, then re-check' };
    const logto = envs[0].urls.logto;
  ...
  async 'glitchtip-token'(values, fetchImpl) {
    ...
    if (gap) return { ok: false, detail: gap };
    const glitchtip = loadStack(stackName('lcl')).urls.glitchtip;
