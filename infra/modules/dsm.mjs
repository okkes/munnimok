...
export const DSM_CODE_ADVICE = {
  ...
  105: 'DSM treats this session as a non-administrator (its own init data says is_admin=false — verify prints it): Control Panel APIs are admin-only. Check User & Group → the deploy user → Edit → User groups → the administrators group is ticked AND saved, then sign in to DSM as this account once in a browser: a 2FA-enrollment or e-mail OTP prompt means Security → Account enforces 2-factor authentication or Adaptive MFA for administrators — exempt the deploy account (or enroll it and give the pipeline its OTP secret)',
