...
const ANNOTATIONS = {
  ...
  'signup-onboarding': {
    ...
    sub: 'Two-step onboarding: Step 1 = profile details; Step 2 = account setup with assets/liabilities groups matching Financial Accounts design.',
    states: [
      { tag: 'step 1 — profile',     note: 'First/last name, email (read-only), country picker, API URL, avatar' },
      { tag: 'step 2 — accounts',    note: 'Assets + liabilities groups; SSO users see default accounts pre-populated; email users start empty' },
    ...
    flows: [
      { from: 'Continue (step 1)',        to: 'Step 2 — account setup' },
      { from: 'Get started (step 2)',     to: 'Login + redirect to home', cond: 'only visible when onboardAccounts is non-empty' },
      { from: 'Skip for now',            to: 'Login + redirect to home', cond: 'only visible when onboardAccounts is empty' },
      ...
      { from: 'Type select → loan',      to: 'LoanFlow (loan type picker → form)' },
      { from: 'Back (step 1)',           to: 'Previous login mode (signup / SSO)' },
      { from: 'Back (step 2)',           to: 'Step 1' },
      { from: 'Back in sub-screens',     to: 'Parent screen (no pushState — browser back goes to step 1)' },
    ],
    rules: [
      ...
      'SSO (Google/Apple): getDefaultAccounts(method) pre-populates onboardAccounts with 3/2 default accounts',
      'Email users: onboardAccounts starts empty; Skip visible only when empty, Get started only when non-empty',
      'On completion, active profile\'s accountIds always synced from onboardAccounts (even when empty)',
      'Account form sub-screens do NOT push history state; browser back from them returns to step 1',
      'PSD2 automated bank sub-screens DO push history (bank-search + bank-credentials); back works correctly',
      'onboardAccounts written to munni_bank_accounts on completion, replacing initPerUserData defaults',
    ],
    edge: [
      'popstate chain: step2 → step1 → previous mode; only PSD2 sub-screens push states',
      'After completion, initPerUserData() must run BEFORE doLogin() to avoid schema overwrite',
      'If user removes all default SSO accounts, they can still proceed with empty onboardAccounts',
    ],
    storage: [
      'munni_profile_firstname / _lastname / _country (LS, per-user key via computeUserDataKey)',
      'munni_api_url (LS)',
      'munni_bank_accounts (LS, per-user key — written with full account objects from onboardAccounts)',
      'munni_user_picture_{method|email} (LS)',
      'munni_signup_draft_{method}_{email} (LS) — persists onboardAccounts + form fields across refreshes',
    ],
  },
  ...
  'home': {
    ...
    rules: [
      ...
      'Nav drawer spaces list has a gradient fade at bottom to hint at scrollability',
      'Manage spaces button lives INSIDE the scrollable area (below last space row), not fixed at bottom',
    ],
    storage: [
  ...
  'txDetail': {
    ...
    flows: [
      ...
      { from: 'Reimbursements card row', to: 'linkReimburse screen (stack) or txDetail of linked TX' },
      { from: 'Category row',            to: 'Category picker sheet' },
      { from: 'Account row (Details)',   to: 'accounts screen (highlights account)' },
      { from: 'Recurring row (Options)', to: 'Link-recurring Sheet' },
      { from: 'Saving account (Options)',to: 'Saving account picker Sheet' },
    ],
  ...
  'accountsAll': {
    ...
    states: [
      ...
      'Edit sheet — name + account number only; no balance field (initial balance is creation-only)',
      'Delete confirm sheet — remove account from assets or liabilities',
    ],
    rules: [
      'Each account row shows a Manual (readOnly=false) or Automated (readOnly=true) badge — no PSD2 label',
      'Account rows have NO delete (X) button — deletion is only via the edit sheet → "Remove account" button',
      'Adding from Settings → Accounts does NOT auto-attach to the active space',
      'Adding from Space detail → Attach Account → Manage Accounts auto-attaches to that specific space',
      'Shared-with-me sheet: Authenticate (automated) launches full PSD2 BankConnectPsd2Screen with IBAN locked (ibanReadOnly=true); on completion account moves to Assets/Liabilities with SHARED+CO-OWNER badges',
      'Shared-with-me sheet: Request co-ownership (manual) opens confirmation sheet; on confirm writes coOwnerRequests to shared data; button becomes "Request sent" badge',
      'Co-owner accounts in Assets/Liabilities show both SHARED and CO-OWNER badges in account row subtitle',
      '"From space" button in shared-with-me sheet and "Shared in" buttons in edit sheet navigate to spaceDetail (not space)',
    ],
  ...
  'spaces': {
    ...
    flows: [
      ...
      { from: 'Space row (entire row)',        to: 'spaceDetail screen (stack) — row no longer activates space directly' },
  ...
  'spaceDetail': {
    ...
    states: [
      ...
      'SETTINGS section — Currency row below avatar/name; opens CurrencyPickerSheet for this space',
      '"Set as active" button — shown below active badge when space is NOT the active space; tapping activates it',
    ],
    flows: [
      ...
      { from: 'Currency row (space-currency-row)', to: 'CurrencyPickerSheet — writes munni_display_currency_{profileId}' },
      { from: 'Add member row (owner)',    to: 'ProfileMembersSheet' },
      { from: 'Member row (owner)',        to: 'MemberActionSheet' },
      { from: 'Change photo',             to: 'photo picker Sheet (5-column sp* icon grid)' },
      { from: '"Set as active" button',   to: 'activates this space (sets profile.active=true for this space, false for all others)' },
    ...
    rules: [
      ...
      'canEdit = myPerm !== "reader" — readers see accounts but cannot attach/detach; currency row is locked (lock icon, opacity 0.5) for readers',
      'isDemo space: attach account sheet shows DEMO_ACCOUNTS (no real bank accounts)',
      'Currency is per-space (munni_display_currency_{profileId}); falls back to global munni_display_currency',
      'Manage Accounts from space attach sheet passes spaceId param → new accounts auto-attach to this space',
      'Default space icon uses sp* IDs (landscape/nature emojis) — user avatars (av*) are for user profiles only',
    ],
    edge: [
    ...
    storage: [
      ...
      'munni_global_users (LS) — display names for member rows (falls back to member.displayName then userId)',
      'munni_global_friendships (LS) — used to detect non-friend members; non-friends shown dimmed with "Not a friend yet" label + quick invite button',
  ...
  'settings': {
    screen: 'Settings',
    sub: 'App-wide preferences — dark mode, language, notifications, app info. Currency is NOT here; it is per-space under Space detail → SETTINGS.',
    flows: [
  ...
  'userInfo': {
    ...
    states: [
      { tag: 'demo',    note: 'Name inputs disabled, no API row, no Delete row — "Demo account" label; country not tappable' },
      { tag: 'google',  note: 'Email row locked (lock icon, no m-tap); API row present; "Signed in with Google" subtitle' },
      { tag: 'apple',   note: 'Email row locked (lock icon, no m-tap); API row present; "Signed in with Apple" subtitle' },
    ...
    flows: [
      { from: 'Name / country fields',   to: 'Auto-save on change — no Save button; navigate back via browser back' },
    ...
    rules: [
      ...
      'isDemo (method=bank): all edits blocked; country btn not tappable; no API/Delete rows',
      'All name/country changes auto-save via useLocalStorage — no Save button exists for any user type',
    ...
    edge: [
      ...
      'Google/Apple onboarding: Step 2 shows pre-populated default accounts (getDefaultAccounts); user can remove/add before proceeding',
    ],
  },
