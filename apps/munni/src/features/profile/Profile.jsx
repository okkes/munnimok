...
export function ScreenSpaceDetail({ params }) {
  ...
  React.useEffect(() => {
    ...
    // Also ensure the owner/creator is always present so invited members can see them
    setSharedData(sd => {
      const updated = { ...(sd.memberPerms || {}) };
      if (!updated[myId]) updated[myId] = 'owner';
  ...
  // Self-repair: if the creator opened a space that already has memberPerms but they're missing from it, add them
  React.useEffect(() => {
    if (!profile || profile.isShared) return; // only the creator (non-shared copy)
    if (!sharedData?.memberPerms) return;
    if (sharedData.memberPerms[myId]) return; // already present
    if (Object.keys(sharedData.memberPerms).length === 0) return; // no members yet
    setSharedData(sd => ({ ...sd, memberPerms: { [myId]: 'owner', ...(sd.memberPerms || {}) } }));
  }, [profile?.id, profile?.isShared, sharedData?.memberPerms?.[myId], !!sharedData?.memberPerms]);
  ...
  // Re-evaluate ALL disconnected PSD2 accounts against every member's personal accounts on space open.
  // This covers the case where a different member (not the one who caused the disconnect) has the account.
  React.useEffect(() => {
    if (!profile?.id) return;
    const sdKey = `munni_shared_data_${profile.id}`;
    let sd;
    try { sd = JSON.parse(localStorage.getItem(sdKey) || '{}'); } catch { return; }
    const disconnectedPsd2 = (sd.accounts || []).filter(a => a.disconnected && a.readOnly);
    if (!disconnectedPsd2.length) return;
    const memberIds = Object.keys(sd.memberPerms || {});
    if (!memberIds.length) return;
    const reconnections = [];
    disconnectedPsd2.forEach(da => {
      const connector = memberIds.find(uid => {
        const keys = getPersonalAcctKeys(uid);
        return keys.some(key => {
          try {
            const accts = JSON.parse(localStorage.getItem(key) || '[]');
            return accts.some(pa => da.iban && pa.iban ? da.iban === pa.iban : da.name === pa.name);
          } catch { return false; }
        });
      });
      if (connector) reconnections.push({ id: da.id, attachedBy: connector });
    });
    if (!reconnections.length) return;
    const reconnectMap = new Map(reconnections.map(r => [r.id, r.attachedBy]));
    const newAccounts = (sd.accounts || []).map(a =>
      reconnectMap.has(a.id)
        ? { ...a, disconnected: false, disconnectedAt: undefined, disconnectedBy: undefined, attachedBy: reconnectMap.get(a.id) }
        : a
    );
    localStorage.setItem(sdKey, JSON.stringify({ ...sd, accounts: newAccounts }));
    window.dispatchEvent(new CustomEvent('munni-ls', { detail: { key: sdKey } }));
  }, [profile?.id]);
    ...
    <div data-testid="space-detail-screen" className="m-screen">
      ...
      <div className="m-body-scroll">
        ...
        {isMemberOfShared ? (
          ...
          /* Owner view: same compact horizontal as member view */
          <div style={{ display:'flex', alignItems:'center', gap:16, padding:'20px 4px 16px' }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:12, color:M.ink3, marginTop:3 }}>{t('space.createdByYou')}</div>
              {isActive ? (
                <div style={{ display:'inline-block', fontSize:10, color:M.sage, fontWeight:700, background:M.sageSoft, padding:'2px 10px', borderRadius:999, marginTop:4 }}>{t('space.active')}</div>
              ) : (
                <button className="m-tap" onClick={() => setProfiles(ps => ps.map(p => ({ ...p, active: p.id === profile.id })))}
                  style={{ fontSize:12, fontWeight:600, color:M.brand, background:'none', border:`1.5px solid ${M.brand}`, padding:'3px 12px', borderRadius:999, cursor:'pointer', fontFamily:M.fontUI, marginTop:4 }}>
                  {t('space.setActive')}
                </button>
              )}
            </div>
        ...
        <div className="m-card" style={{ padding:'4px 16px', marginBottom:16, border:`1px solid ${M.line}` }}>
          ...
          <Divider inset={44}/>
          {(() => {
            const sharedPeriodDay = isProfileShared ? sharedData?.meta?.periodDay : null;
            const sharedPeriodType = isProfileShared ? sharedData?.meta?.periodType : null;
            const effectivePeriodDay = sharedPeriodDay ?? profile.periodDay ?? 20;
            const effectivePeriodType = sharedPeriodType ?? profile.periodType ?? 'monthly';
            const periodLabel = effectivePeriodType === 'weekly' ? 'Weekly' : effectivePeriodType === 'biweekly' ? 'Bi-weekly' : `Day ${effectivePeriodDay}`;
            return (
              <div className={canEdit ? 'm-tap' : ''} onClick={canEdit ? () => nav.push('periods', { profileId: profile.id }) : undefined}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  <span style={{ fontSize:13, color:M.ink2, fontWeight:600 }}>{periodLabel}</span>
                  {canEdit ? <I name="caretR" size={14} color={M.ink4}/> : <I name="lock" size={13} color={M.ink4}/>}
                </div>
              </div>
            );
