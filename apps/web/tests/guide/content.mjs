...
export const GUIDE = [
  ...
  },
  {
    id: 'home',
    title: 'Home is yours',
    body: `Home is a landing zone of blocks: review queue, this period, new transactions, budgets, upcoming costs and more. Features you haven't tried yet gather in one compact Explore block instead of filling the page — tap a row to start; each disappears once it's in use, and Explore is a block like any other, so Customize Home orders or hides it too. Reorder or hide blocks with Customize Home at the bottom. The avatar on the top right switches spaces.`,
  ...
  },
  {
    ...
    body: `The review deck shows one transaction at a time with a suggested category and the reason behind it. Everything you change — category, counterparty, splits — stays a draft until you hit Confirm. "Also apply to similar" catches the rest of the same merchant in one go.`,
    tips: ['Tap the description to read the full bank text.', 'Skip is honest: it leaves no trace and the card returns later.', 'Categories carry the meaning: a plain category files the money by its sign, and the marked ◆ special categories tell the special stories — Set aside, a loan payment, a transfer, money to the shared funding pot. Picking one asks the counterparty on the spot: the space\\u2019s default pot with one tap, or a real account.', 'On a savings, loan or investment account the story is locked: the account itself types every one of its rows, and the sign reads the direction (+ on a loan = repaid).', 'Splitting CATEGORIES is not splitting the transaction: the category chip spreads one payment across several categories — exact euros or percentages — and the row stays ONE transaction, listed once with a thin color bar. Each ◆ entry answers its own counterparty right under its row — €60 groceries next to €40 Set aside into a real pot, even a funding leg in the same spread.', 'The Split transaction row is the other gear: the amounts partition into parts that stack like cards in a deck; tap a peeking card and it shuffles to the front to take its categories, counterparty, recurring cost and event right there.', 'Parts are full transactions and nothing restricts them beyond the amounts adding up — five expense parts, two savings, any mix is a real split. Splitting a filled transaction asks first: the row becomes a pure container and its own category, recurring link, event and note reset — the parts carry those from then on. Confirm stays tappable while parts are incomplete; tapping it marks exactly which parts still need a category.', 'Missing a category, recurring cost or event? Create it right from the picker — the card keeps your place.'],
    shots: ['13-review-banner', '14-review-flow', '15-review-done'],
  },
  {
    id: 'transactions',
    title: 'Transaction details',
    body: `Open any transaction to recategorize, split it, name a counterparty (one of your tracked accounts makes it a transfer, and a manual counterpart gets its own leg written automatically; on manual rows a small Adjustment toggle marks corrections), attach receipts, link recurring costs or events, and record reimbursements that show the net cost. Splitting has two gears — and they are different features. SPLIT CATEGORIES is the light one: the category editor spreads one payment across several categories, exact euros or percentages, and the row stays ONE transaction with everything a transaction has — and every ◆ special entry names its own counterparty the moment you pick it (a saving leg to one pot, a funding leg to another, plain spending beside them; the pot moves by exactly that entry's amount). SPLIT TRANSACTION is the heavy one — when one payment carries several kinds of money: each part has its own name ("Albert Heijn – split 1", rename freely), its own counterparty, even its own category spread — the €100 phone bill can be €50 telecom and €50 paying the device loan off, and the loan moves by exactly its €50; the container itself keeps no category at all. Settled value moves into the special Reimbursed category — real categories keep only what you truly paid — and the transactions tab has an "Unsettled reimbursements" quick filter for everything still waiting on money.`,
    tips: ['Search matches amounts too: typing 10 finds 10.99 and 210.15 — and part names, so "Sarah\\u2019s loan" finds the payment that carries it.', 'A split shows its sub-transactions as rows of their own in the list, branching off a compact header that names the original payment with its full amount — the chevron folds them away; each row opens as its own page with its own type, categories, event, note and reimbursements, its sisters one tap away, and the whole transaction one more.', 'Filters see through splits: when a category or type filter matches only some parts, exactly those parts show — aligned like any other row, wearing a small split mark — in the list and in the overview drill-downs alike.', 'Amount fields in the split editor do math: type 87,40-25 and leave the field.', 'Amount fields also type like a register: 5 means 0,05, then 55 is 0,55, then 550 is 5,50 — or type the comma yourself, both work.', 'Labeling a row "Loan payment" asks where it went: the space\\u2019s default loan pot with one tap ("Default \\u2014 no setup"), or a real loan account \\u2014 and a manual account can simply point at the row that is already there instead of writing a new one.', 'A transfer between two of your accounts shows as ONE row ("Checking → Savings"); filtering by account brings each leg back.', 'Recategorizing offers a bulk apply — tap the bar to pick exactly which transactions it touches. Splitting offers the same: the split lands on the picked same-name rows, resized to each amount.', 'Rename a transaction via the pencil — munni remembers and auto-renames future arrivals; the bank original stays under Details.', 'Linking a reimbursement opens its own screen: search by name or amount, or take the suggestion — munni spots likely matches by timing, wording and size.'],
  ...
  },
  {
    ...
    tips: ['Push notifications tell you when new bank transactions arrive — tapping one opens that space\\u2019s review directly.', 'The sync card at the top of Settings shows the last successful sync.', 'Profile → Devices lists every device signed in to your account — rename them, and disconnect one remotely: it erases its munni data the next time it connects.'],
