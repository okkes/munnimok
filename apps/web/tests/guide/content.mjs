...
export const GUIDE = [
  ...
  },
  {
    ...
    tips: ['Tap the description to read the full bank text.', 'Skip is honest: it leaves no trace and the card returns later.', 'The card leads with the kind: Standard files itself as income or expense by the sign; Transfer strictly needs one of your tracked accounts as the counterparty — the special stories (set aside without a pot, a loan payment, money to the shared account) live on the marked ◆ special categories instead.', 'On a savings, loan or investment account the kind is locked: the account itself types every one of its rows, and the sign reads the direction (+ on a loan = repaid).', 'Splitting sits in the open: a Split row on every card sets the amounts — exact euros or percentages — and the parts stack like cards in a deck, the others peeking full-width from behind; tap one (or step with the ‹ › arrows — ten parts stay walkable) and it swaps to the top to take its type, category, event and note right there. Each part can spread its own money across several categories too. Manage splits reshapes the amounts.', 'Parts are full transactions, so honest repeats are fine — five expense parts is a real split. What is refused, on the spot, is the same story told twice: two transfers to the same account, or the same special kind (◆) twice — that is one part, not two. Nothing half-lands: from the detail screen a split stays a draft until every part is complete, then applies in one go.', 'Missing a category, recurring cost or event? Create it right from the picker — the card keeps your place.'],
