...
export const GUIDE = [
  ...
  },
  {
    ...
    tips: ['Tap the description to read the full bank text.', 'Skip is honest: it leaves no trace and the card returns later.', 'The card leads with the kind: Standard files itself as income or expense by the sign; Transfer strictly needs one of your tracked accounts as the counterparty — the special stories (set aside without a pot, a loan payment, money to the shared account) live on the marked ◆ special categories instead.', 'On a savings, loan or investment account the kind is locked: the account itself types every one of its rows, and the sign reads the direction (+ on a loan = repaid).', 'Splitting sits in the open: a Split row on every card sets the amounts — exact euros or percentages — and the parts stack like cards in a deck; tap a peeking card and it shuffles to the front to take its type, categories, recurring cost and event right there. A part\\u2019s category row opens the SAME editor a whole transaction has — several categories, euros or percentages, scoped to that part.', 'Parts are full transactions and nothing restricts them beyond the amounts adding up — five expense parts, two savings, any mix is a real split. Splitting a filled transaction asks first: the row becomes a pure container and its own category, recurring link, event and note reset — the parts carry those from then on. Confirm stays tappable while parts are incomplete; tapping it marks exactly which parts still need a category.', 'Missing a category, recurring cost or event? Create it right from the picker — the card keeps your place.'],
    shots: ['13-review-banner', '14-review-flow', '15-review-done'],
  },
  {
    id: 'transactions',
    title: 'Transaction details',
    body: `Open any transaction to recategorize, SPLIT IT, set its kind (Standard, Transfer or — on manual rows — Adjustment; a Transfer strictly links one of your tracked accounts, and a manual counterpart gets its own leg written automatically), attach receipts, link recurring costs or events, and record reimbursements that show the net cost. Splitting has two gears. Spreading one payment across several categories is the light one: add categories in the editor, exact euros or percentages, done. "Split into parts" is the heavy one — when one payment carries several kinds of money: each part has its own name ("Albert Heijn – split 1", rename freely), its own type, even its own category spread — the €100 phone bill can be €50 telecom and €50 paying the device loan off, and the loan moves by exactly its €50. Settled value moves into the special Reimbursed category — real categories keep only what you truly paid — and the transactions tab has an "Unsettled reimbursements" quick filter for everything still waiting on money.`,
