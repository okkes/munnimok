...
export const GUIDE = [
  ...
  },
  {
    ...
    tips: ['Tap the description to read the full bank text.', 'Skip is honest: it leaves no trace and the card returns later.', 'The card leads with the kind: Standard files itself as income or expense by the sign; Transfer strictly needs one of your tracked accounts as the counterparty — the special stories (set aside without a pot, a loan payment, money to the shared account) live on the marked ◆ special categories instead.', 'On a savings, loan or investment account the kind is locked: the account itself types every one of its rows, and the sign reads the direction (+ on a loan = repaid).', 'Splitting sits in the open: a Split row on every card sets the amounts — exact euros or percentages — and the parts stack like cards in a deck, the others peeking from behind; tap one to bring it on top and set its type, category, event and note right there (the main card steps back to just the money). Manage splits reshapes the amounts.', 'A split exists to carry SEVERAL kinds of money — two parts of the same kind are refused on the spot, and a split born in the values flow will not confirm until every kind differs; spread one kind across categories with Add category instead. Nothing half-lands: from the detail screen a split stays a draft until every part is complete, then applies in one go.', 'Missing a category, recurring cost or event? Create it right from the picker — the card keeps your place.'],
