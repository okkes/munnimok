...
describe('PayPal funding auto-link (PP1 rung 2)', () => {
  ...
  async function seeded() {
    ...
    await repo.upsert('transaction', SPACE, 'bank-2', { accountId: 'acc-bank', date: '2026-07-14', amountCents: -700, currency: 'EUR', merchant: 'Albert Heijn', needsReview: 0, catId: 'groceries' });
    return { store, repo };
  }
  ...
  it('does nothing without a PayPal feed in the space', async () => {
    ...
    await repo.upsert('account', SPACE, 'acc-bank', { name: 'Checking', type: 'checking', source: 'manual', currency: 'EUR', balanceCents: 0 });
    await repo.upsert('transaction', SPACE, 'bank-1', { accountId: 'acc-bank', date: '2026-07-14', amountCents: -2599, currency: 'EUR', merchant: 'PayPal Europe', needsReview: 1 });
    expect(await linkPaypalFunding(store, repo, SPACE)).toBe(0);
  });
});
