...
/** scripts the AH endpoints; first list call can demand a token refresh */
function fakeAh({ expireFirst = false, refreshWorks = true } = {}) {
  let refreshed = false;
  const calls: string[] = [];
  const call: ProxyCall = async (_store, path, init) => {
    ...
    if (path === '/mobile-services/v2/receipts') {
      ...
      return {
        status: 200,
        json: [
          { transactionId: 't-100', transactionMoment: '2026-07-05T17:31:00Z', total: { amount: { amount: 23.5 } } },
          { transactionId: 't-200', transactionMoment: '2026-07-03T09:00:00Z', total: { amount: { amount: 9.99 } } },
        ],
      };
    }
    if (path.startsWith('/mobile-services/v2/receipts/')) {
      return {
        status: 200,
        json: { receiptUiItems: [{ type: 'product', description: 'MELK', amount: '2,58' }] },
      };
    }
