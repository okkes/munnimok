...
async function resolveTargets(store: StorageBackend, connection: StoreConnectionRow, activeSpaceId: string): Promise<string[]> {
...
async function markExpired(store: StorageBackend, repo: Repo, connection: StoreConnectionRow, activeSpaceId: string): Promise<void> {
  await store.storeConnPut({ ...connection, status: 'expired' });
...
async function ingestSpace(
  ...
  const [allTxs, existing] = await Promise.all([
...
export async function syncAhReceipts(call: ProxyCall, store: StorageBackend, repo: Repo, activeSpaceId: string): Promise<StoreSyncResult> {
  const connection = await store.storeConnGet('ah');
  ...
  if (list.status === 401) {
    ...
    if (!refreshed) {
      await markExpired(store, repo, connection, activeSpaceId);
  ...
  const targets = await resolveTargets(store, connection, activeSpaceId);
  ...
  for (const spaceId of targets) {
    added += await ingestSpace(store, repo, spaceId, 'ah', 'Albert Heijn', receipts, fetchItems);
  }

  await store.storeConnPut({
    ...connection,
...
export async function syncJumboReceipts(call: ProxyCall, store: StorageBackend, repo: Repo, activeSpaceId: string): Promise<StoreSyncResult> {
  const connection = await store.storeConnGet('jumbo');
  if (connection?.status !== 'ok') return { status: 'error', added: 0 };
  ...
  let added = 0;
  for (const spaceId of targets) {
    added += await ingestSpace(store, repo, spaceId, 'jumbo', 'Jumbo', list.receipts, fetchItems);
  }
