...
public static class LogoEndpoints
{
    ...
    public static void MapLogos(this IEndpointRouteBuilder app, IConfiguration config)
    {
        ...
        app.MapGet("/logos/health", async (IHttpClientFactory http, CancellationToken ct) =>
        {
            ...
            if (!configured)
            {
                search = "unconfigured — set LOGODEV_SECRET_KEY (sk_…) and LOGODEV_PUBLIC_TOKEN (pk_…)";
            }
            else
            {
                using var client = http.CreateClient(HttpClientName);
                using var response = await client.GetAsync("search?q=netflix", ct);
                search = response.IsSuccessStatusCode
                    ? "ok"
                    : response.StatusCode is System.Net.HttpStatusCode.Unauthorized or System.Net.HttpStatusCode.Forbidden
                        ? "unauthorized — the SECRET key is wrong (or the pk_/sk_ keys are swapped)"
                        : $"upstream error {(int)response.StatusCode}";
            }
